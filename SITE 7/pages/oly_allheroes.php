<?php if(!$indexing || $dpage['olyall'] != 1) { echo "<script>document.location.replace('./');</script>"; exit; } ?>
...
$cacheFile = "cache/oly_allheroes.xml";
$genNew = 0;
...
if($genNew) {
	...
	if(count($query) > 0) {
		...
		for($i=0, $c=count($query); $i < $c; $i++) {
			...
			$line .= "<count>".$query[$i]['count']."</count>\n";
...
if((intval($dpage['olyrak'])+intval($dpage['olyher'])+intval($dpage['olyall'])) > 1) {
	...
	<div class='horMenu'>
		...
		".($dpage['olyall'] == 1 ? "<a href='#ollyLoc' class='act'>".$LANG[12025]."</a>" : "")."
	</div>
	";
}
